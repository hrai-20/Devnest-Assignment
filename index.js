...
async function scrapeAmazon(keyword) {
  ...
  const products = await page.evaluate(() => {
    ...
    firstFourItems.forEach(async (item) => {
      ...
      const descriptionElement = item.querySelector("h2.a-size-mini.a-color-base");
      const ratingElement = item.querySelector(".a-icon-star-small");
      const reviewsElement = item.querySelector(".a-size-base.a-color-secondary");
      const priceElement = item.querySelector(".a-price span");

      const productName = nameElement ? nameElement.innerText.trim() : "N/A";
      const productDescription = descriptionElement ? descriptionElement.innerText.trim() : "N/A";
      const productRating = ratingElement ? ratingElement.innerText.split(" ")[0] : "N/A";
      ...
      productList.push({ productName, productDescription, productRating, productReviews, productPrice, innerUrl });
    });
    return productList;
  });
  ...
  const updatedProducts = await Promise.all(
    products.map(async (product) => {
      ...
      const { topReviews, reviewCount } = await page.evaluate(() => {
        const reviewItems = document.querySelectorAll('div[data-hook="review"]');
        const reviewItemsArray = Array.from(reviewItems);
        const topReviews = reviewItemsArray.map((item) => {
          return item.querySelector(".a-size-base.review-text.review-text-content").innerText;
        });
        const reviewElement = document.querySelector(".a-row.a-spacing-base.a-size-base");
        const reviews = reviewElement ? reviewElement.innerHTML.split(", ")[1].split(" ")[0] : "N/A";
